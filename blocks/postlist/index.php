...
function gpg_render_block_latest_postlist( $attributes, $content ) {
	...
	foreach ( $recent_posts as $post ) {
        ...
        $list_item_markup .= sprintf( '<div class="gpg-list-post">' );

            // Get the featured image
            if ( isset( $attributes['displayPostImage'] ) && $attributes['displayPostImage'] ) {
                if ( has_post_thumbnail( $post_id ) ) {
                    $list_item_markup .= sprintf(
                        '<div class="gpg-post-list-image"><a href="%1$s" rel="bookmark">%2$s</a></div>',
                        esc_url( $titlelink ),
            ...
            $list_item_markup .= sprintf( '<div class="gpg-list-post-content">' );

            // Category.
            if ( isset( $attributes['displayCategory'] ) && $attributes['displayCategory'] ) {
                $list_item_markup .= sprintf(
                    '<div class="gpg-category-grid">%1$s</div>',
            ...
            if ( isset( $attributes['displayPostTitle'] ) && $attributes['displayPostTitle'] ) {
                $list_item_markup .= sprintf(
                    '<'.$headingSelector.' class="gpg-post-list-title"><a href="%1$s">%2$s</a></'.$headingSelector.'>',
                    esc_url( $titlelink ),
                    esc_html( $title )
                );
            }
            ...
            if ( $attributes['displayPostAuthor'] || $attributes['displayPostDate'] || $attributes['displayCategory'] || $attributes['displayComments'] ) {
                $list_item_markup .= sprintf( '<div class="gpg-list-post-meta">' );
                if ( isset( $attributes['displayPostAuthor'] ) && $attributes['displayPostAuthor'] ) {

                    $list_item_markup .= sprintf(
                        '<span class="gpg-post-list-author"><a target="_blank" href="%1$s">%2$s</a></span>',
                ...
                if ( isset( $attributes['displayPostDate'] ) && $attributes['displayPostDate'] ) {
                    $list_item_markup .= sprintf(
                        '<span class="gpg-post-list-date"><time datetime="%1$s">%2$s</time></span>',
            ...
            if ( isset( $attributes['displayPostLink'] ) && $attributes['displayPostLink'] ) {
                ...
                $list_item_markup .= sprintf(
                    '<div class="gpg-post-list-link"><a href="%1$s" rel="bookmark">%2$s</a></div>',
                    esc_url( get_permalink( $post_id ) ),
                    esc_html( $readMore )
                );
            }
            $list_item_markup .= sprintf( '</div>' );//post content
        $list_item_markup .= sprintf( '</div>' );
    }//end foreach
    ...
    if ( ! empty( $attributes['catMargin'] ) ) {
        $mainStyles[] = '--gpg-cat-margin: ' . $attributes['catMargin'].'px';
    }
    ...
    $block_content = sprintf(
        '<div class="gpg-list-post-item" style="%1$s">%2$s</div>',
 ...
 */
function gpg_blocks_register_rest_fields_list() {
...
add_action( 'rest_api_init', 'gpg_blocks_register_rest_fields_list' );
...
function gpg_register_dynamic_post_list() {
	register_block_type(
		'blocks-post-grid/postlist',
		array(
			'attributes' => array(
                'id' => array (
                    'type' => 'string',
			...
			'render_callback' => 'gpg_render_block_latest_postlist',
		)
	);
}
add_action( 'init', 'gpg_register_dynamic_post_list' );